...
# =========================================================
# GLOBAL CSS OVERRIDES (STUNNING SAAS AESTHETIC & FIXES)
# =========================================================
...
/* Completely Eliminate Streamlit Unwanted Border Outlines */
...
.upload-info-text .main-lbl {{
    font-size: 15px;
    font-weight: 800;
    color: var(--text);
    line-height: 1.2;
}}
.upload-info-text .sub-lbl {{
    font-size: 11.5px;
    color: var(--text-muted);
    margin-top: 3px;
    font-weight: 500;
...
/* Streamlit File Uploader Hack Overlay */
...
/* Streamlit Selectbox & Popover High-Contrast Fixes (Fixes Bug 1) */
...
/* BaseWeb Popover Dropdown Color Fix */
...
if not st.session_state.quiz_started:
    ...
    with col_left:
        left_card_html = textwrap.dedent(f"""
            ...
            <div class="drop-zone-wrapper">
                <div class="cloud-graphic-box">
                    <svg width="140" height="96" viewBox="0 0 140 96" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M 28 50 A 42 42 0 0 1 112 50" stroke="{T['cloud_accent']}" stroke-width="2" stroke-dasharray="3 5" opacity="0.8" fill="none"/>
                        <path d="M 40 62 C 31 62 26 55 26 47 C 26 39 32 33 41 33 C 44 24 53 17 65 17 C 78 17 87 25 89 34 C 96 34 102 40 102 47 C 102 55 96 62 88 62 Z" 
                              fill="{T['cloud_bg']}" stroke="{T['cloud_stroke']}" stroke-width="2.5" stroke-linejoin="round"/>
                        <rect x="53" y="30" width="26" height="32" rx="4" fill="{T['surface']}" stroke="{T['cloud_stroke']}" stroke-width="2.5"/>
                        <path d="M 60 39 H 72 M 60 46 H 72 M 60 53 H 68" stroke="{T['cloud_stroke']}" stroke-width="2" stroke-linecap="round"/>
                        <path d="M 70 50 L 75 44 C 76.8 42.2 79.8 42.2 81.6 44 C 82.8 45.2 82.8 47 81.6 48.2 L 74 57 L 74 66 L 67 66 L 67 54 Z" 
                              fill="{T['cloud_accent']}" stroke="{T['surface']}" stroke-width="1.2"/>
                    </svg>
                </div>
                
                <div class="upload-footer-flex">
                    <div class="upload-info-text">
                        <div class="main-lbl">Drag and drop file here</div>
                        <div class="sub-lbl">Limit 200MB per file • PDF, DOCX</div>
                    </div>
                    <div class="browse-pill-btn">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
                            <path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/>
                            <path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/>
                            <path d="M8 6h8 M8 10h8"/>
                        </svg>
                        Browse files
                    </div>
                </div>
    ...
    with col_right:
        right_header_html = textwrap.dedent("""
        <div class="saas-card-box">
            <div class="card-title-row">
                <span class="dot-indicator"></span>
        ...
        slider_meta_html = textwrap.dedent(f"""
...
if st.session_state.quiz_started and len(st.session_state.questions) > 0 and not st.session_state.quiz_finished:
    ...
    q_header_html = textwrap.dedent(f"""
        <div style="font-size:12px; font-weight:800; text-transform:uppercase; letter-spacing:0.08em; color:var(--text-muted); margin-bottom:6px;">
    ...
    if st.session_state.show_result:
        ...
        fb_html = textwrap.dedent(f'<div style="background:var(--surface-2); border-left:4px solid var(--accent); padding:16px; border-radius:12px; font-weight:500;">{result.get("feedback","")}</div>')
        st.markdown(fb_html, unsafe_allow_html=True)

        st.markdown("#### Explanation")
        exp_html = textwrap.dedent(f'<div style="background:var(--surface-2); border-left:4px solid var(--accent); padding:16px; border-radius:12px; font-weight:500;">{explanation}</div>')
